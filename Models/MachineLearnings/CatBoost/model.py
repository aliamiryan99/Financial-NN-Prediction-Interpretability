...
def create_sequences(features, target, seq_length):
    """Prepare sequences for CatBoost model."""
    X = []
    y = []
    for i in range(len(features) - seq_length):
        # Flatten the sequence for CatBoost
...
def build_model():
    """Build the CatBoost model using global parameters."""
...
def run(config: Config):
    ...
    # Prepare sequences
    print("Step 5: Preparing the Data for CatBoost")
    ...
    # Build model
    print("Step 6: Building the CatBoost Model")
    model = build_model()
